...
function lexer( pstridl )

  if not stridllen then
    init()
    stridllen = string.len( pstridl )
    stridl = pstridl
  end --if
  ...
  while true do
    ...
    --dummy char    
      ...
      if ( lookahead == ' ' ) then
        line_new = getchar( stridl )
        lookahead = getchar( stridl )
        while ( is_digit( lookahead ) ) do
          line_new = line_new..lookahead
          lookahead = getchar( stridl )
        end 
        line = tonumber( line_new )
      --'"' char
        _ = getchar( stridl )
        lookahead = getchar( stridl )
        srcfilename_new = ''
        while ( lookahead ~= '"'  ) do
          srcfilename_new = srcfilename_new..lookahead
          lookahead = getchar( stridl )
        end
        srcfilename = srcfilename_new
        lookahead = getchar( stridl ) 
      -- flags
        while ( lookahead == ' ' ) do
          lookahead = getchar( stridl )
          if is_digit( lookahead ) then
            --tab_flags[ lookahead ] = true
            linemark[lookahead] = true
          end --if
          lookahead = getchar( stridl )
        end --while
        table.insert( tab_linemarks, linemark )
  ...
  -- formatting character ??
    elseif lookahead == '\'' then
      ...
      if lookahead == '\\' then
        ...
        -- When to occur a unknown escape sequence, then we applie a common
        -- behavior that is return a proper character. Ex.: '\e' -> e
          tokenvalue = tokenvalue..lookahead
          lookahead = getchar( stridl )
        end -- if
      elseif lookahead ~= '\'' then 
