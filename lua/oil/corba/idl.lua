...
local match  = strlib.match
local format = strlib.format

local ArrayedSet = require "loop.collection.ArrayedSet"
local add = ArrayedSet.add
...
function setnameof(contained, name)
	...
	if container then
		...
		if not start then
			illegal(container.repID, "parent scope repository ID")
		end
...
function Contents:_remove(contained)
	contained = remove(self, contained)
	if contained then
	...
	if not isinstanceof(self.definitions, Contents) then
		...
		if self.definitions then
			for _, value in ipairs(self.definitions) do
				asserttype(value.name, "string", "IDL definition name")
...
function Contained(self)
	...
	asserttype(self.name, "string", "IDL definition name")
	asserttype(self.repID, "string", "repository ID")
	return self
end
...
function union(self)
	...
	asserttype(self.switch, "idl type", "union type discriminant")
	asserttype(self.options, "table", "union options definition")
	asserttype(self.default, "number", "union default option definition")
	
	self.selector = {} -- maps field names to labels (option selector)
	self.selection = {} -- maps labels (option selector) to options
	for index, option in ipairs(self.options) do
		checkfield(option)
		if option.label == nil then illegal(nil, "option label value") end
...
function enum(self)
	...
	asserttype(self.enumvalues, "table", "enumeration values definition")
	
	self.labelvalue = {}
	for index, label in ipairs(self.enumvalues) do
		asserttype(label, "string", "enumeration value label")
...
function sequence(self)
	...
	asserttype(self.maxlength, "number", "sequence type maximum length")
	asserttype(self.elementtype, "idl type", "sequence element type")
	return self
end
...
function array(self)
	...
	if self.elementtype == nil then self.elementtype = self[1] end
	asserttype(self.elementtype, "idl type", "array element type")
	return self
end
...
function attribute(self)
	...
	asserttype(self.type, "idl type", "attribute type")

	local mode = self.mode
	if mode == "ATTR_READONLY" then
		self.readonly = true
	elseif mode ~= nil and mode ~= "ATTR_NORMAL" then
		illegal(self.mode, "attribute mode")
	end
...
function operation(self)
	...
	elseif mode ~= nil and mode ~= "OP_NORMAL" then
		illegal(self.mode, "operation mode")
	end
	...
	if self.parameters then
		for _, param in ipairs(self.parameters) do
			checkfield(param)
			if param.mode then
				asserttype(param.mode, "string", "operation parameter mode")
				...
				elseif param.mode == "PARAM_INOUT" then
					...
					illegal(param.mode, "operation parameter mode")
				end
			else
				self.inputs[#self.inputs+1] = param.type
			end
	...
	if self.exceptions then
		for _, except in ipairs(self.exceptions) do
			asserttype(except, "idl except", "raised exception")
			if self.exceptions[except.repID] ~= nil then
				illegal(except.repID,
